...
#                 # st.chat_message("Model").markdown(response['answer'])
...
# st.set_page_config(page_title="Global Guru", layout="wide")
...
#     # Debug print to check chunk structure
#     logging.info(f"Processing chunk: {chunk}")
#     return FAISS.from_documents(chunk, embeddings)
...
#                     futures = [executor.submit(process_document_chunk, chunk, embeddings) for chunk in chunks]
#                     results = [future.result() for future in futures]
                
#                 # Debug print to check results
#                 logging.info(f"Results: {results}")

#                 vectors = FAISS.from_documents([item for sublist in results for item in sublist], embeddings)
...
#     if groqApi_provided:
#         st.sidebar.success("✅ Groq API key is already provided.")
#     else:
#         groqApi = st.sidebar.text_input("Enter Your Groq API Key", type="password")
#         if st.sidebar.button("Submit Groq API Key"):
#             save_api_keys(groqApi, api_keys.get("geminiApi"))
#             st.sidebar.success("✅ Groq API key submitted.")
    
#     if geminiApi_provided:
#         st.sidebar.success("✅ Gemini API key is already provided.")
#     else:
#         geminiApi = st.sidebar.text_input("Enter Your Gemini API Key", type="password")
#         if st.sidebar.button("Submit Gemini API Key"):
#             save_api_keys(api_keys.get("groqApi"), geminiApi)
#             os.environ["GOOGLE_API_KEY"] = geminiApi
#             st.sidebar.success("✅ Gemini API key submitted.")
...
#     # Button to create document embeddings
#     with st.sidebar.expander("Document Embeddings", expanded=True):
#         st.sidebar.markdown("<hr>", unsafe_allow_html=True)
...
#                 st.sidebar.success("✅ Good To Go.")
#         else:
#             st.sidebar.success("✅ Document embeddings are already created.")
...
#                 st.session_state.messages.append({"role": "bot", "content": f"<b>Hello! 🌟 I'm Global Guru, your personal legal advisor. I’m here to assist you with your legal queries.</b>\n\nHere's the information you need:\n\n{answer}"})
#                 st.experimental_rerun()  # Forces a rerun to update the chat
#             except Exception as e:
#                 st.error(f"Error retrieving answer: {str(e)}")
#     else:
#         st.info("Please provide API keys and create document embeddings to start using the chatbot.")

#     st.sidebar.markdown("<hr>", unsafe_allow_html=True)
#     st.sidebar.markdown("<h2 class='sidebar-footer'>Developed with ❤️ for GenAI by <a href='https://www.linkedin.com/in/aryan-tiwari-174a50298/' style='text-decoration:none;'>Aryan Tiwari</a></h2>", unsafe_allow_html=True)

# else:
#     st.sidebar.warning("Please provide both API keys to access the chatbot functionality.")
